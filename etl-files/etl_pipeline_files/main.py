...
from etl_pipeline.config.database import get_db_config
from etl_pipeline.config.api_config import APIConfig
from etl_pipeline.utils.logger import setup_logger
from etl_pipeline.utils.date_utils import generate_date_chunks
from etl_pipeline.utils.idempotency import IdempotencyChecker
from etl_pipeline.extract.crimes_extractor import CrimesExtractor
from etl_pipeline.extract.persons_extractor import PersonsExtractor
from etl_pipeline.extract.property_extractor import PropertyExtractor
from etl_pipeline.extract.interrogation_extractor import InterrogationExtractor
from etl_pipeline.extract.mo_seizures_extractor import MoSeizuresExtractor
from etl_pipeline.extract.chargesheets_extractor import ChargesheetsExtractor
from etl_pipeline.extract.fsl_case_property_extractor import FslCasePropertyExtractor
from etl_pipeline.load.files_loader import FilesLoader
...
class ETLPipeline:
    ...
    def initialize(self):
        ...
        try:
            ...
            # Load API config
            self.logger.info("Loading API configuration from api-ref.txt...")
            self.api_config = APIConfig('api-ref.txt')
    ...
    def process_crimes(self, start_date='2022-01-01', end_date=None):
        ...
        # Check for last processed date for crimes API
        last_dates = self.get_last_processed_date_per_api()
        if 'crime' in last_dates:
            resume_date = last_dates['crime']
            self.logger.info(f"📌 Found last processed date for crimes: {resume_date}")
            # Subtract 1 day to account for overlap and ensure no data is missed
            from datetime import timedelta
            resume_dt = datetime.strptime(resume_date, '%Y-%m-%d')
            resume_date_minus_overlap = (resume_dt - timedelta(days=1)).strftime('%Y-%m-%d')
            self.logger.info(f"   Resuming from {resume_date_minus_overlap} (last processed: {resume_date}, accounting for 1-day overlap)")
            # Use resume_date_minus_overlap as start_date, but ensure it's not after end_date
            if end_date is None:
                end_date = datetime.now().strftime('%Y-%m-%d')
            if resume_date_minus_overlap < end_date:
                start_date = resume_date_minus_overlap
            else:
                self.logger.info(f"   Last processed date ({resume_date}) is >= end_date ({end_date}), skipping")
                return
        else:
            self.logger.info(f"📌 No previous file IDs found for crimes - starting from {start_date}")
        
        # Generate date chunks with 1-day overlap
        if end_date is None:
            end_date = datetime.now().strftime('%Y-%m-%d')
        
        chunks = generate_date_chunks(start_date, end_date, chunk_days=5, overlap_days=1)
        self.logger.info(f"Processing {len(chunks)} date chunks from {start_date} to {end_date}")
    ...
    def process_persons(self):
        ...
        for idx, person_id in enumerate(person_ids, 1):
            ...
            try:
                ...
                # Extract files
                files = extractor.extract_files(data, person_id=person_id)
    ...
    def process_property(self, start_date='2022-01-01', end_date=None):
        ...
        # Check for last processed date for property API
        last_dates = self.get_last_processed_date_per_api()
        if 'property' in last_dates:
            resume_date = last_dates['property']
            self.logger.info(f"📌 Found last processed date for property: {resume_date}")
            # Subtract 1 day to account for overlap and ensure no data is missed
            from datetime import timedelta
            resume_dt = datetime.strptime(resume_date, '%Y-%m-%d')
            resume_date_minus_overlap = (resume_dt - timedelta(days=1)).strftime('%Y-%m-%d')
            ...
            self.logger.info(f"📌 No previous file IDs found for property - starting from {start_date}")
    ...
    def process_interrogation(self, start_date='2022-01-01', end_date=None):
        ...
        # Check for last processed date for interrogation API
        last_dates = self.get_last_processed_date_per_api()
        if 'interrogation' in last_dates:
            resume_date = last_dates['interrogation']
            self.logger.info(f"📌 Found last processed date for interrogation: {resume_date}")
            # Subtract 1 day to account for overlap and ensure no data is missed
            from datetime import timedelta
            resume_dt = datetime.strptime(resume_date, '%Y-%m-%d')
            resume_date_minus_overlap = (resume_dt - timedelta(days=1)).strftime('%Y-%m-%d')
            ...
            self.logger.info(f"📌 No previous file IDs found for interrogation - starting from {start_date}")
    ...
    def process_mo_seizures(self, start_date='2022-01-01', end_date=None):
        ...
        # Check for last processed date for mo_seizures API
        last_dates = self.get_last_processed_date_per_api()
        if 'mo_seizures' in last_dates:
            resume_date = last_dates['mo_seizures']
            self.logger.info(f"📌 Found last processed date for mo_seizures: {resume_date}")
            # Subtract 1 day to account for overlap and ensure no data is missed
            from datetime import timedelta
            resume_dt = datetime.strptime(resume_date, '%Y-%m-%d')
            resume_date_minus_overlap = (resume_dt - timedelta(days=1)).strftime('%Y-%m-%d')
            ...
            self.logger.info(f"📌 No previous file IDs found for mo_seizures - starting from {start_date}")
    ...
    def process_chargesheets(self, start_date='2022-01-01', end_date=None):
        ...
        # Check for last processed date for chargesheets API
        last_dates = self.get_last_processed_date_per_api()
        if 'chargesheets' in last_dates:
            resume_date = last_dates['chargesheets']
            self.logger.info(f"📌 Found last processed date for chargesheets: {resume_date}")
            # Subtract 1 day to account for overlap and ensure no data is missed
            from datetime import timedelta
            resume_dt = datetime.strptime(resume_date, '%Y-%m-%d')
            resume_date_minus_overlap = (resume_dt - timedelta(days=1)).strftime('%Y-%m-%d')
            ...
            self.logger.info(f"📌 No previous file IDs found for chargesheets - starting from {start_date}")
    ...
    def process_fsl_case_property(self, start_date='2022-01-01', end_date=None):
        ...
        # Check for last processed date for fsl_case_property API
        # Note: source_type in files table might be 'case_property' not 'fsl_case_property'
        last_dates = self.get_last_processed_date_per_api()
        if 'case_property' in last_dates:
            resume_date = last_dates['case_property']
            self.logger.info(f"📌 Found last processed date for fsl_case_property: {resume_date}")
            # Subtract 1 day to account for overlap and ensure no data is missed
            from datetime import timedelta
            resume_dt = datetime.strptime(resume_date, '%Y-%m-%d')
            resume_date_minus_overlap = (resume_dt - timedelta(days=1)).strftime('%Y-%m-%d')
            ...
            self.logger.info(f"📌 No previous file IDs found for fsl_case_property - starting from {start_date}")
