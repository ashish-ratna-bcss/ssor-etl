...
Run this from: /data-drive/etl-process-dev/etl-files/etl_files_media_server/
"""
import os
import sys
...
# Try to import config (should be in same directory as this script)
try:
    from config import DB_CONFIG
except ImportError:
    # Fallback: load from .env directly
...
# Import mapping function from files media server main.py
try:
    from main import map_destination_subdir
except ImportError:
    # If running from different location, define mapping function directly
    def map_destination_subdir(source_type: str, source_field: str):
        """Map (source_type, source_field) to destination subdirectory"""
        ...
        return mappings.get((source_type, source_field))
