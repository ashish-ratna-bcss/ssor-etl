...
class UpdatedChargesheetETL:
    ...
    def pending_crime_ids(self) -> List[str]:
        """crime_ids already in crimes but with no updated-chargesheet rows
        fetched yet -- SSOR branch: driven off crimes (GET
        /update-chargesheets/{crimeId}), not an independent date-range
        scan."""
        self._cursor.execute(f"""
            SELECT c.crime_id FROM {CRIMES_TABLE} c
            WHERE NOT EXISTS (
                SELECT 1 FROM {UPDATE_CHARGESHEET_TABLE} u WHERE u.crime_id = c.crime_id
            )
        """)
        return [row[0] for row in self._cursor.fetchall()]
    ...
    def fetch_updated_chargesheet_by_crime_id(self, crime_id: str) -> Optional[List[Dict]]:
        """GET /update-chargesheets/{crimeId} -- SSOR branch: updated
        chargesheets is driven off crimes, not an independent date-range
        scan."""
        url = f"{API_CONFIG['base_url']}/update-chargesheets/{crime_id}"
        headers = {'x-api-key': API_CONFIG['api_key']}

        for attempt in range(API_CONFIG['max_retries']):
            try:
                logger.debug(f"Fetching updated chargesheet for crime_id {crime_id} (Attempt {attempt + 1})")
                ...
                if response.status_code == 200:
                    ...
                    if data.get('status'):
                        ...
                        if chargesheet_data:
                            if isinstance(chargesheet_data, dict):
                                chargesheet_data = [chargesheet_data]
                            logger.info(f"✅ Fetched {len(chargesheet_data)} updated chargesheet records for crime_id {crime_id}")
                            return chargesheet_data
                        else:
                            logger.warning(f"⚠️  No updated chargesheet records found for crime_id {crime_id}")
                            return []
                    else:
                    ...
                    logger.warning(f"⚠️  No updated chargesheet found for crime_id {crime_id} (404)")
                    return []
        ...
        logger.error(f"❌ Failed to fetch updated chargesheet for crime_id {crime_id} after {API_CONFIG['max_retries']} attempts")
        return None
    ...
    def process_crime_id(self, crime_id: str, table_columns: Set[str] = None):
        """Process updated chargesheet records for a single crime_id (GET
        /update-chargesheets/{crimeId}) -- SSOR branch: driven off crimes.
        Log helpers below only use from_date/to_date as display labels."""
        ...
        chargesheet_raw = self.fetch_updated_chargesheet_by_crime_id(crime_id)
    ...
    def run(self):
        ...
        try:
            ...
            # SSOR branch: updated chargesheets is driven off crime_ids
            # already in crimes (GET /update-chargesheets/{crimeId}), not an
            ...
            logger.info(f"📊 Found {len(crime_ids)} crime_ids pending updated-chargesheet fetch")
            logger.info("=" * 80)
            logger.info("")
