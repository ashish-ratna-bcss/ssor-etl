...
-- Migration: Normalize ADDITIONAL_DETAILS and MEDIA, enforce null/type consistency
-- Safety: Idempotent, no destructive drops, suitable for phased rollout

BEGIN;

-- 1) Parent table adjustments
DROP MATERIALIZED VIEW IF EXISTS public.firs_mv CASCADE;

ALTER TABLE public.properties
    ALTER COLUMN date_of_seizure DROP DEFAULT,
    ALTER COLUMN date_created DROP DEFAULT,
    ALTER COLUMN date_modified DROP DEFAULT,
    ALTER COLUMN estimate_value DROP DEFAULT,
    ALTER COLUMN recovered_value DROP DEFAULT;

-- Normalize timestamp columns to timestamptz.
-- Assumption: existing timestamp without zone values represent IST, matching the API source.
ALTER TABLE public.properties
    ALTER COLUMN date_of_seizure TYPE timestamptz
    USING CASE
        WHEN date_of_seizure IS NULL THEN NULL
        ELSE date_of_seizure AT TIME ZONE 'Asia/Kolkata'
    END,
    ALTER COLUMN date_created TYPE timestamptz
    USING CASE
        WHEN date_created IS NULL THEN NULL
        ELSE date_created AT TIME ZONE 'Asia/Kolkata'
    END,
    ALTER COLUMN date_modified TYPE timestamptz
    USING CASE
        WHEN date_modified IS NULL THEN NULL
        ELSE date_modified AT TIME ZONE 'Asia/Kolkata'
    END;

-- Ensure numeric fidelity for value fields.
ALTER TABLE public.properties
    ALTER COLUMN estimate_value TYPE numeric(15,2)
    USING estimate_value::numeric,
    ALTER COLUMN recovered_value TYPE numeric(15,2)
    USING recovered_value::numeric;

-- Recreate firs_mv after the dependent property columns have been altered.
...
-- Normalize known blank-string fields to NULL (defensive and idempotent).
UPDATE public.properties
SET recovered_from = NULL
WHERE recovered_from IS NOT NULL
  AND btrim(recovered_from) = '';

UPDATE public.properties
SET place_of_recovery = NULL
WHERE place_of_recovery IS NOT NULL
  AND btrim(place_of_recovery) = '';

-- Required performance index for incremental ETL.
CREATE INDEX IF NOT EXISTS idx_properties_date_modified
    ON public.properties (date_modified);

-- 2) Child table for ADDITIONAL_DETAILS (canonical nested object store)
CREATE TABLE IF NOT EXISTS public.property_additional_details (
    property_id varchar(50) PRIMARY KEY,
    additional_details jsonb NOT NULL,
    date_created timestamptz,
    date_modified timestamptz,
    CONSTRAINT property_additional_details_json_is_object
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'property_additional_details_property_fk'
          AND conrelid = 'public.property_additional_details'::regclass
    ) THEN
        ALTER TABLE public.property_additional_details
            ADD CONSTRAINT property_additional_details_property_fk
...
CREATE INDEX IF NOT EXISTS idx_property_additional_details_date_modified
    ON public.property_additional_details (date_modified);

CREATE INDEX IF NOT EXISTS idx_property_additional_details_gin
    ON public.property_additional_details USING gin (additional_details);

-- 3) Child table for MEDIA array
CREATE TABLE IF NOT EXISTS public.property_media (
    ...
    date_created timestamptz,
    date_modified timestamptz,
    CONSTRAINT property_media_unique_entry UNIQUE (property_id, media_index)
);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'property_media_property_fk'
          AND conrelid = 'public.property_media'::regclass
    ) THEN
        ALTER TABLE public.property_media
            ADD CONSTRAINT property_media_property_fk
            FOREIGN KEY (property_id)
            REFERENCES public.properties(property_id)
            ON DELETE CASCADE;
    END IF;
...
CREATE INDEX IF NOT EXISTS idx_property_media_file_id
    ON public.property_media (media_file_id);

CREATE INDEX IF NOT EXISTS idx_property_media_date_modified
    ON public.property_media (date_modified);

-- 4) Backfill child table data from existing parent JSONB columns
-- ADDITIONAL_DETAILS backfill (skip null and non-object payloads).
INSERT INTO public.property_additional_details (
    property_id,
    additional_details,
    date_created,
    date_modified
...
WHERE p.additional_details IS NOT NULL
  AND jsonb_typeof(p.additional_details) = 'object'
...
-- MEDIA backfill (supports scalar media IDs and object payloads).
...
SELECT
    p.property_id,
    (m.ordinality - 1)::integer AS media_index,
    CASE
        WHEN jsonb_typeof(m.value) = 'string' THEN NULLIF(btrim(trim(both '"' FROM m.value::text)), '')
        WHEN jsonb_typeof(m.value) = 'object' THEN COALESCE(
            NULLIF(btrim(m.value ->> 'fileId'), ''),
            NULLIF(btrim(m.value ->> 'file_id'), ''),
            NULLIF(btrim(m.value ->> 'mediaFileId'), ''),
            NULLIF(btrim(m.value ->> 'MEDIA_FILE_ID'), ''),
            NULLIF(btrim(m.value ->> 'id'), '')
        )
        ELSE NULL
    END AS media_file_id,
    CASE
        WHEN jsonb_typeof(m.value) = 'object' THEN COALESCE(
            NULLIF(btrim(m.value ->> 'url'), ''),
            NULLIF(btrim(m.value ->> 'mediaUrl'), ''),
            NULLIF(btrim(m.value ->> 'MEDIA_URL'), ''),
            NULLIF(btrim(m.value ->> 'fileUrl'), ''),
            NULLIF(btrim(m.value ->> 'file_url'), '')
        )
        ELSE NULL
    END AS media_url,
    m.value AS media_payload,
    p.date_created,
    p.date_modified
FROM public.properties p
